...
export async function getStaticProps({ params }: GetStaticPropsContext<ParsedUrlQuery>) {
	...
	const firsCategoryItem = firstLevelMenu.find(m => m.route == params.type);
	if (!firsCategoryItem) {
		return {
			notFound: true
		};
	...
	try {
		const { data: menu } = await axios.post<TopPageProps[]>(process.env.NEXT_PUBLIC_DOMAIN + '/api/top-page/find', {
